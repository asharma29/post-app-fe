	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				console.log("🚀 ~ file: UsersTable.jsx ~ line 18 ~ fetchData ~ res", res)
				setUsers({ loading: false, error: false, data: res });
			} catch (error) {
	...
	const filterData = (data) => {
		...
		if (filters.hasOwnProperty('userName') && filters.userName)
			temp = [
				...temp,
				...data.filter((el) =>
					el.name.toLowerCase().includes(filters.userName.toLowerCase()),
				),
			];
		...
		if (checkBoxFilters && checkBoxFilters.length > 0) {
			...
			temp = [
				...temp,
				...data.filter((el) =>
					checkBoxFilters.includes(el.entryNum.toLowerCase()),
				),
			];
